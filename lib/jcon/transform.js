...
exports.addProperties = function (ctx, f) {
    var id = f.directives.reduce(function (prev, prop) {
        return ((prop instanceof ast.Property) && (prop.path[0].value === 'id')) ?
            prop.value.value : prev;
    }, path_1.basename(ctx.path));
    var idProp = new ast.Property([new ast.Identifier('id', {})], new ast.StringLiteral(id, {}), {});
    var selfProp = new ast.Property([
        new ast.Identifier('app', {}),
        new ast.Identifier('dirs', {}),
        new ast.Identifier('self', {}),
    ], new ast.StringLiteral(getRelPath(ctx.root, ctx.path), {}), {});
    return new ast.File(f.includes, __spreadArrays([idProp, selfProp], f.directives), {});
};
var getRelPath = function (root, dir) {
    var path = path_1.normalize(dir.split(root).join(''));
    return (path === '') ? '/' : path;
};
/**
