 ...
 */
exports.newContext = function (path, loader) {
...
exports.compile = function (ctx, file) {
    return future_1.doFuture(function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, transform_1.transformTree(ctx, file)];
                case 1:
                    f = _a.sent();
                    return [2 /*return*/, future_1.pure(exports.wrapDirectives(ctx, f.directives, exports.file2TS(ctx, f)))];
            }
        });
    });
};
...
exports.getAllImports = function (dirs) {
    return dirs.reduce(function (p, c) { return (c instanceof ast.Property) ?
        addImports(p, c.value) : p; }, {});
};
var addImports = function (imps, node) {
...
var addImportsInList = function (imps, node) {
    return node.elements.reduce(addImports, imps);
};
var addImportsInDict = function (imps, node) {
    return node.properties.reduce(function (p, c) { return addImports(p, c.value); }, imps);
};
/**
...
exports.file2TS = function (ctx, f) {
    // Filter out comments.
    var props = f.directives.filter(function (d) {
...
exports.value2TS = function (ctx, n) { return match_1.match(n)
    ...
    .caseOf(ast.Function, function (n) { return n.body; })
...
var member2TS = function (ctx, m) {
    return imports_1.normalize(m.module.module) + "." +
        (m.invocation ?
...
var list2TS = function (ctx, node) {
    return "[" + node.elements.map(function (n) { return exports.value2TS(ctx, n); }).join(',') + "]";
};
var dict2TS = function (ctx, node) {
    return exports.flattenCodeStruct(ctx, structFromDict(ctx, node));
};
