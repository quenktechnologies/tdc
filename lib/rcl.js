...
exports.compile = exports.parse = exports.file2TS = exports.imports2TS = exports.file2Imports = exports.newContext = void 0;
...
var addQualifiedImport = function (p) { return function (_a) {
    ...
    return record_1.merge(p, (_b = {}, _b[module.value] = id.value, _b));
}; };
/**
...
exports.imports2TS = function (i) {
    return record_1.reduce(i, [], function (p, c, k) {
        return Array.isArray(c) ? __spreadArrays(p, ["import { " + c.join(',') + " } from '" + k + "';"]) : __spreadArrays(p, ["import * as " + c + " from '" + k + "';"]);
    }).join(os_1.EOL);
};
/**
...
var mergeRs = function (f) { return function (list) {
    return list.reduce(function (p, c) {
        p.routes = __spreadArrays(c.routes, p.routes);
        return p;
    }, f);
}; };
...
var route2TS = function (r) {
    return "{ method: " + method2TS(r.method) + "," +
        ("path: " + pattern2TS(r.pattern) + ",") +
        "filters: " + (r.view ?
        "[" + (r.filters.length > 0 ? filters2TS(r.filters) + ',' : '') +
            (view2TS(r.view) + "]);" + os_1.EOL) :
        "[" + filters2TS(r.filters) + "]}" + os_1.EOL);
};
var filters2TS = function (filters) {
    return filters.map(filter2TS).join(',');
};
