 ...
 */
const newContext = (loader) => record_1.merge({ loader, rcl: exports.parse, locals: [] }, {});
exports.newContext = newContext;
/**
 * parse source text into an rcl File node.
 */
const parse = (src) => future_1.fromCallback(cb => {
 ...
 */
const compile = (src, ctx) => exports.parse(src).chain(f => exports.file2TS(ctx, f));
 ...
 */
const file2TS = (ctx, node) => future_1.doFuture(function* () {
 ...
 * [[ast.File]].
 */
const resolveIncludes = (ctx, node) => future_1.doFuture(function* () {
...
const functionCall2TS = ({ id, args }) => {
    ...
    if ((!isMember && parts.length > 1) || (isMember && parts.length > 2)) {
        ...
        let isConstructor = array_1.tail(parts) === array_1.tail(parts).toUpperCase();
        return isConstructor ? `new ${path}(${argsStr})` :
            `${path}.call(${target}, [${argsStr}])`;
    }
    else {
        let isConstructor = array_1.tail(parts)[0] === array_1.tail(parts)[0].toUpperCase();
