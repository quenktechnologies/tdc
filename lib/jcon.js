...
var var2Ts = function (n) {
    return n.filters.reduce(function (p, c) { return literal2TS(c.name) + "(" + p + ")"; }, exports.value2TS(n.key));
};
var envVar2Ts = function (n) {
    return n.filters.reduce(function (p, c) { return literal2TS(c.name) + "(" + p + ")"; }, "(<string>process.env['" + exports.value2TS(n.key) + "'])");
};
...
exports.compile = function (src, ctx) {
    return exports.parse(src).chain(function (f) {
        return exports.file2TS(ctx, f).chain(wrapOutput(ctx, f));
    });
};
