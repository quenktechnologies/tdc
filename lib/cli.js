...
exports.exec = function (path, opts) {
    return assertExists(path)
        ...
        .chain(function () { return opts.noStart ? future_1.pure(undefined) : exports.writeStartFile(path, opts); });
};
...
var compile = function (path, opts) { return function (_a) {
...
var addCreate = function (f) {
    ...
    var prop = new jconAst.Property(path, new jconAst.ArrowFunction(os_1.EOL + "//@ts-ignore: 61333 " + os_1.EOL +
        "(app:App) => new Module(app)", loc), loc);
    f.directives.unshift(prop);
    return f;
};
var combine = function (ctx, conf, routes, opts) {
